...
typedef CONST CHAR8 *ANSI_COLOR;

typedef struct {
  UINT32        Level;
  ANSI_COLOR    Color;
} DEBUG_COLOR;
static DEBUG_COLOR mDebugColor[] =
{
  { DEBUG_ERROR,         ANSI_FG_RED                },   // Red
  { DEBUG_WARN,          ANSI_RGB_FG(255, 255,   0) },   // Yellow
//  { DEBUG_INFO,          ANSI_RGB_FG(  0, 255,   0) },   // White
  { DEBUG_INFO,          ANSI_FG_GREEN              },   // Green
  { DEBUG_VERBOSE,       ANSI_RGB_FG(160, 160, 160) },   // Gray
  { DEBUG_LOAD,          ANSI_RGB_FG(  0, 255, 255) },   // Cyan
  { DEBUG_DISPATCH,      ANSI_RGB_FG(  0, 255, 255) },   // Cyan
  { DEBUG_INIT,          ANSI_RGB_FG(  0, 255,   0) },   // Green
  { DEBUG_FS,            ANSI_RGB_FG(255,   0, 255) },   // Magenta
  { DEBUG_POOL,          ANSI_RGB_FG(255, 128,   0) },   // Orange
};
...
{
	UINTN     Index;
  for (Index = 0 ; Index < sizeof(mDebugColor) / sizeof(DEBUG_COLOR) ; Index++) {
    if (ErrorLevel & mDebugColor[Index].Level)
      return mDebugColor[Index].Color;
  }

  return  ANSI_RGB_FG(105,105,105); // Light White
}
  ...
  )
{
  ...
  DebugVPrint (ErrorLevel, GetDebugColor((UINT32)ErrorLevel), NULL);
  DebugVPrint (ErrorLevel, Format, Marker);
  DebugVPrint (ErrorLevel, ANSI_FG_DEFAULT, NULL);
  VA_END (Marker);
  ...
  )
{
  ...
  AsciiSPrint (
    Buffer, 
    sizeof (Buffer), 
    ANSI_FG_BRIGHT_YELLOW ANSI_BG_RED"ASSERT [%a] %a(%d):"ANSI_FG_RED ANSI_BG_DEFAULT" %a\n"ANSI_FG_DEFAULT, 
    gEfiCallerBaseName, 
    FileName, 
    LineNumber, 
    Description
